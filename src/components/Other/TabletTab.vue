...
<!-- ══════════════════════════════════════════════════════════════
      ...
      <div v-if="hienThe" v-loading="loading" class="flex-1 min-h-0 overflow-y-auto p-3">
        <div v-if="paginatedTablets.length > 0" class="grid grid-cols-1 gap-4">
          <div
            v-for="(row, i) in (paginatedTablets as any[])"
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <!-- ══════════════════════════════════════════════════
                     ...
                     Lý do đầy đủ ở `PhoneTab.vue` cùng MỤC. Tóm tắt: giữ
                     MỘT cột `os_version`, chỉ đổi cách nhập — thêm cột
                     `os_type` riêng là có hai nguồn cho cùng một sự thật.

                     ⚠️ Máy tính bảng có thêm "iPadOS" so với điện thoại.
                     Bỏ nó là mọi iPad phải khai vào mục "Khác".
                     ══════════════════════════════════════════════════ -->
...
//
// Database vẫn chỉ có MỘT cột `os_version`. Hai ô trên giao diện chỉ là
// cách nhập — `he_dieu_hanh` và `phien_ban_os` KHÔNG gửi lên máy chủ.
//
// 🔴 THỨ TỰ TRONG DANH SÁCH DƯỚI QUAN TRỌNG: "ipados" phải đứng TRƯỚC
// "ios". Xét "ios" trước thì chuỗi "iPadOS 17" không khớp (nó bắt đầu
// bằng "ipa"), nhưng nếu ai đó đổi thành so bằng `includes` thì "iPadOS"
// sẽ bị nhận nhầm thành "iOS". Giữ thứ tự này là chặn sẵn.
// ══════════════════════════════════════════════════════════════════════
const ghepOsVersion = () => {
  form.os_version = `${form.he_dieu_hanh || ''} ${form.phien_ban_os || ''}`.trim()
}

const NHAN_OS: Record<string, string> = {
  android: 'Android', ipados: 'iPadOS', ios: 'iOS',
}
