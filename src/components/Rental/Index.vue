...
<div id="hdg-noi-dung-rental" class="h-full"></div>
    </el-splitter-panel>
    </el-splitter>
...
<div id="hdg-noi-dung-rental" class="h-full"></div>
  ...
  <!-- ══════════════════════════════════════════════════════════════════
       ...
       ⚠️ `defer` LÀ BẮT BUỘC. Không có nó, Teleport đi tìm `#hdg-noi-dung-rental`
       ...
       ══════════════════════════════════════════════════════════════════ -->
  <Teleport defer to="#hdg-noi-dung-rental">
...
// Tự động kích hoạt hiệu ứng loading chuyển tab trong 300ms và tự động redirect nếu không thuộc về rental
watch(
...
const activeMenu = computed({
  get() {
    const subview = (route.params.subview as string) || 'real-estate'
    return subviewMap[subview] || '1-1'
  },
  set(val) {
    const subview = indexMap[val] || 'real-estate'
    router.push(`/rental/${subview}`)
  }
})
...
const currentMenuItem = computed(() =>
  sidebarMenuItems.find(item => item.index === activeMenu.value) || { index: '1-1', label: 'Quản lý Bất động sản', icon: OfficeBuilding }
)
