<template>
  ...
  <div v-else class="h-full flex" :class="isMobile ? 'flex-col' : 'flex-row'">
    <template v-if="isMobile">
      <!-- Toggle bar displaying current selected menu -->
      <div
    ...
    <!-- TABLET (768 – 1023px): Collapsed Sidebar showing only icons -->
...
const activeMenu = computed({
  get() {
    return (route.params.subview as string) || 'players'
  },
  set(val) {
    router.push(`/rosca/${val}`)
  }
})
...
const currentMenuItem = computed(() =>
  sidebarMenuItems.find(item => item.index === activeMenu.value) || { index: 'players', label: 'Người chơi', icon: User }
)
