  ...
  <div class="members-content h-full flex flex-col text-left">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden flex flex-col flex-1 min-h-0">
      ...
      >
        ...
        <!-- Mã Giao dịch ID -->
        <el-table-column label="Mã giao dịch" width="300" show-overflow-tooltip>
...
<!-- ══════════════════════════════════════════════════════════════
      ...
      <div v-if="hienThe" v-loading="loading" class="flex-1 min-h-0 overflow-y-auto p-3">
        <div v-if="paginatedContributions.length > 0" class="grid grid-cols-1 gap-4">
          <div
            v-for="(row, i) in (paginatedContributions as any[])"
          ...
          >
            ...
            <div class="space-y-2 text-sm text-left">
              ...
              <div class="flex justify-between gap-3">
                <span class="text-gray-400 dark:text-gray-500 font-medium shrink-0">Ngày đóng thực tế:</span>
                <span class="text-right break-words min-w-0">
                  <span class="text-xs text-gray-500 dark:text-gray-400 font-mono">
                                {{ formatDate(row.actual_payment_date) }}
                              </span>
